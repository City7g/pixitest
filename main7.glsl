...
void main(void) {
    ...
    float adjustedMixFactor = mix(1.0, transform(uProgress, 1.0, 1.4), (1.0 - distanceFromCenter));
    
    // Применение smoothstep для плавного перехода
    float mixFactor = smoothstep(0.0, 1.0, uProgress * 1.7 - distance(coord, vec2(0.5, 0.5)));
